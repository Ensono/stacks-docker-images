...
curl -L https://dl.k8s.io/release/${KUBE_VERSION}/bin/linux/${BIN_ARCH}/kubectl -o /usr/local/kubectl/bin/kubectl 
...
curl -L https://get.helm.sh/helm-v${HELM_VERSION}-linux-${BIN_ARCH}.tar.gz -o /tmp/helm.tar.gz 
...
curl -L "https://github.com/kluctl/kluctl/releases/download/${KLUCTL_VERSION}/kluctl_${KLUCTL_VERSION}_linux_${BIN_ARCH}.tar.gz" -o /tmp/kluctl.tar.gz 
...
curl -L "https://github.com/kubernetes-sigs/kustomize/releases/download/kustomize/${KUSTOMIZE_VERSION}/kustomize_${KUSTOMIZE_VERSION}_linux_${BIN_ARCH}.tar.gz" -o /tmp/kustomize.tar.gz 
