...
A pipeline has been configured for Azure DevOps, which calls `eirctl`, and runs the tasks.
...
| `debug` | Turn on debug output for `eirctl` | icon:fas-times[]
| `stages` | A list of objects that define what pipeline from `eirctl` needs to be run.
...
| {src_docs_path}/foundation/builder.adoc | {dst_docs_path}/foundation/builder/README.md
...
| {src_docs_path}/bottlerocket-cis-bootstrap.adoc | {dst_docs_path}/bottlerocket-cis-bootstrap/README.md
| {src_docs_path}/bottlerocket-cis-validation.adoc | {dst_docs_path}/bottlerocket-cis-validation/README.md
...
| {src_docs_path}/node.adoc | {dst_docs_path}/node/README.md
|===
...
The process that manages the generation of the README files is a PowerShell script that is called by Eirctl as part of the build.

.Task for generating Docker overview files
[options=header,stripes=even]
|===
| Eirctl Task | PowerShell Script | Script Reference
| `generate:docker:overview` | `build/scripts/Set-DockerOverview.ps1` | <<Set-DockerOverview Script>>
|===

NOTE: The `generate:docker:overview` is set as part of the Eirctl `docs` pipeline
