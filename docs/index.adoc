...
`golang:1.192.2-alpine3.16` | Used to build our Go based applications, such as https://github.com/amido/stacls-cli[Stacks CLI] | <<Golang Image>>
...
This is the case if running in ADO or on a local machine. The only dependency that needs to be installed beforehand is the AmidoBuild PowerShell module and it needs to be added to your `PSModulePath` environment variable.
